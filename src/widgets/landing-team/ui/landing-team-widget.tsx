...
export const LandingTeamWidget = () => {
  ...
  return (
    <section
      ref={containerRef}
      className="py-24 bg-transparent border-t border-gray/40"
    >
      <div className="max-w-[1400px] mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-p-sm text-lighter-gray mt-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {TEAM_MEMBERS.map((member, index) => (
            <div
              key={index}
              className="team-card group p-6 rounded-xl bg-secondary-gray/80 backdrop-blur-md border border-gray hover:border-blue transition-all duration-300 hover:-translate-y-1"
            >
              <div className="relative w-full aspect-square rounded-lg overflow-hidden bg-terciary-gray mb-4">
                <Image
                  src={member.image}
                  alt={member.name}
                  fill
              ...
              <p className="text-xs text-lighter-gray mt-1">{member.role}</p>
            </div>
          ))}
        </div>
      </div>
